...
MegaCoreTargetLowering::MegaCoreTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(MegaCore::R1);
  ...
  setOperationAction(ISD::SDIV, MVT::i32, Legal);      // для srem
...
#include "MegaCoreGenCallingConv.inc"
...
SDValue MegaCoreTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_MegaCore);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, MegaCore::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(MegaCoreISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_MegaCore);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const MegaCoreSubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const MegaCoreTargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// MegaCore formal arguments implementation
SDValue MegaCoreTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_MegaCore);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &MegaCore::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    MegaCoreFunctionInfo *UFI = MF.getInfo<MegaCoreFunctionInfo>();
...
bool MegaCoreTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_MegaCore))
    return false;
...
SDValue
MegaCoreTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
  ...
  const MegaCoreSubtarget &STI = MF.getSubtarget<MegaCoreSubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_MegaCore);
  ...
  return DAG.getNode(MegaCoreISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue MegaCoreTargetLowering::PerformDAGCombine(SDNode *N,
...
bool MegaCoreTargetLowering::isLegalAddressingMode(const DataLayout &DL,
